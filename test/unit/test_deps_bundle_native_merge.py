...
name no matter how many Python versions VRED might embed. ``scripts/deps_bundle.py``
...
def merged_bundle(tmp_path, supported_versions) -> Path:
    ...
    sentinel of its own, standing in for a copy resolved for the build host's interpreter.
    """
...
def test_colliding_abi3_artifact_comes_from_the_lowest_supported_abi(
    ...
    Console sign-in reports that sign-in is needed, indefinitely. With 3.11 the only
    supported version that gets an abi3 wheel, the collision here is with the base
    environment's host-resolved copy, which must lose to 3.11's tree.
    """
