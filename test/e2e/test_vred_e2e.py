...
from test.common.constants import Constants as TestConstants
from test.common.vred_runner import VREDRunner
from test.common.job_bundle_output_comparison import assert_job_bundle_matches
from test.common.sticky_settings_verification import verify_sticky_settings_file
from test.common.output_comparison import are_images_similar_by_folder
from test.e2e.path_resolver import PathResolver

logging.basicConfig(format="%(message)s", level=logging.INFO)


@pytest.fixture(scope="module", autouse=True)
def setup_and_cleanup_e2e_output():
    """
    Module-scoped fixture to clean up output directory before and after running the tests.
    """
    output_dir = Path(__file__).parent / "output"

    logging.info("Cleaning up e2e test output directory...")
    try:
        if output_dir.exists():
            shutil.rmtree(output_dir)
        output_dir.mkdir(exist_ok=True)
    except (OSError, PermissionError) as e:
        logging.warning(f"Could not clean e2e test output directory: {e}")

    yield

    logging.info("Cleaning up e2e test output directory after tests...")
    try:
        if output_dir.exists():
            shutil.rmtree(output_dir)
    except (OSError, PermissionError) as e:
...
class E2ETestRunner:
    """
    Class to run end-to-end tests for VRED Deadline Cloud integration
    """
    ...
    def run_e2e_test(
        ...
        """
        Run an end-to-end test which does following:
        ...
        logging.info(f"Scene: {scene_basename}.vpb")
        logging.info(f"Bundle: {bundle_path}")
        logging.info(f"Output: {render_output_dir}")
        ...
        expected_output_folder = self.path_resolver.get_expected_output_render_folder(
    ...
    def _validate_template(self, template_path: Path) -> None:
        ...
        try:
            ...
            raise RuntimeError(f"Invalid template YAML: {e}")
    ...
    def validate_submitter_outputs(
        ...
        expected_bundle_dir = self.path_resolver.get_expected_output_bundle_folder(
            test_name, scene_basename
        )
...
@pytest.mark.e2e
class TestVREDE2E:
    def test_e2e_gpu_raytracing(self):
        ...
        render_output, expected_render_output, bundle_path, sticky_settings_file = (
            runner.run_e2e_test("gpu_raytracing", "Cone.vpb", test_settings)
        )
