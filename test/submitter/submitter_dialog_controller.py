...
and trigger job bundle export for submitter testing.
"""
...
class SubmitterDialogController:
    ...
    def set_job_specific_settings(self, settings_list) -> bool:
        ...
        try:
            ...
            # overriding any explicit GPU Ray Tracing setting provided
            ...
            if "StartFrame" in settings and "EndFrame" in settings:
                ...
                frame_step = settings.get("FrameStep", 1)
                if frame_step == 1:
                    frame_range_text = f"{settings['StartFrame']}-{settings['EndFrame']}"
                else:
                    frame_range_text = (
                        f"{settings['StartFrame']}-{settings['EndFrame']}x{frame_step}"
                    )
                self.scene_settings_widget.frame_range_widget.setText(frame_range_text)
            if "View" in settings:
...
def run_submitter_integration_test(test_settings: Dict[str, Any], bundle_output_path: str) -> bool:
    ...
    param: bundle_output_path: directory path to where the job bundle should be exported
