...
class Constants(metaclass=ConstantsMeta):
    ...
    ERROR_VRED_PRO_REQUIRED: Final[str] = (
        "GUI tests require VRED Pro. Please set the VREDPRO environment variable to point to a "
        "valid VRED Pro executable. VRED Core cannot be used for GUI testing as it is headless."
    )
    ...
    # Invoke code found in get_bootstrap_code() (indirectly via CODE_PASSING_ENV_VAR)
    # without using import statements. Removes all spaces to prevent VRED from assuming
    # non-intended arguments.
