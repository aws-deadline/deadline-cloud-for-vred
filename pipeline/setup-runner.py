...
def wait_for_concurrent_install(marker_file, timeout=300):
    """Wait for a concurrent installation to complete."""
...
def install_vred_version(version):
    ...
    lock_file = Path(f"C:/Temp/vred-{version}.lock")
    lock_file.parent.mkdir(parents=True, exist_ok=True)

    if lock_file.exists():
        print(f"Waiting for concurrent VRED {version} install...")
        wait_for_concurrent_install(marker_file)
        return
