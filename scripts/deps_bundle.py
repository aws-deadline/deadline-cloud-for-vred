...
# VRED Pro's embedded interpreters, not this package's own build/CI interpreter: VRED 2025
# (17.3) embeds Python 3.10, VRED 2026 (18.0) embeds 3.11 (see the StrEnum backport in
# vred_submitter/utils.py). pyproject.toml's `requires-python` floor and `Python :: 3.12`
# classifier describe the build/CI interpreter only -- no VRED release embeds 3.12.
...
#
# awscrt: wheels are not uniformly abi3 -- 3.10 gets a version-specific
# _awscrt.cpython-310-<platform>.so, 3.11+ gets the shared _awscrt.abi3.so.
...
def _build_base_environment(working_directory: Path, dependencies: list[str]) -> Path:
    ...
    # Requested here (not in project.dependencies) since only the submitter bundle needs the
    # console extra; requesting it lets awscrt track whatever version botocore's crt extra
    # pins, rather than resolving independently and drifting.
...
def _download_native_dependencies(working_directory: Path, base_env: Path) -> list[Path]:
    # No --platform flag: these downloads resolve for the build host's own platform, which
    # _copy_native_to_base_env's merge now makes load-bearing rather than a gap-fill. Safe only
    # because the in-app submitter this bundle ships for is Windows-only (install_submitter.py
    # rejects non-Windows) and release_publish.yml's BuildInstaller job hardcodes
    # `oses: ['Windows']`, so the CodeBuild host for a shipped release is always Windows.
