...
# the base environment would ship whichever the build host produced, so any VRED whose
...
# falling back to its pure-Python parser on the other one.
...
def _build_base_environment(working_directory: Path, dependencies: list[str]) -> Path:
    ...
    # The bundle is the submitter, which needs AWS Console sign-in. Only the submitter
    # bundle needs the console extra, and it pulls awscrt, a compiled wheel, so it is
    # requested here at bundle time rather than declared in project.dependencies.
    #
...
def _copy_native_to_base_env(base_env: Path, native_dependency_paths: list[Path]) -> None:
    ...
    collision is a no-op. ``awscrt`` publishes a separate abi3 wheel per Python, each
